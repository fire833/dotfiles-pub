...
function setwin; i3-msg 'workspace 10; exec keepassxc; exec bash -c "yubiauth"'; end
function crawl -a url; wget -r -p -U mozilla -E -e robots=off --random-wait -np --limit-rate 1M $url; end 
function wificheck -a interface; sudo iw dev $interface scan | grep SSID; end
# Download videos as 1080p mp4
function ytdl -a url; yt-dlp -v -f "bv*[height<=1080][ext=mp4]+ba*[ext=m4a]" -N 4 $url; end
# Download videos as mp3 audio
function ytdla -a url; yt-dlp -v -f "ba" -x --audio-format mp3 -N 4 $url; end

# Export an env var without printing stuff to stdout
function setsilent -a varname
    echo "Provide the string for var $varname: "
    read value

    set -x $varname $value;
end

function setvaultok; setsilent VAULT_TOKEN; end
function setpulumipw; setsilent PULUMI_CONFIG_PASSPHRASE; end
...
function clearenv; 
    ...
    set --erase PULUMI_CONFIG_PASSPHRASE
